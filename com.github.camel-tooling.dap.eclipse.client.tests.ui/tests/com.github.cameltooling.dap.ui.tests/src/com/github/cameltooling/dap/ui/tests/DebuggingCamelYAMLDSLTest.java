...
public class DebuggingCamelYAMLDSLTest {
	...
	public static final String PROJECT_FOLDER_PATH = "resources/camel-examples/main-yaml";

	/**
	 * Sets Generic Editor as default editor for yaml. Imports project and creates
	 * run/debug configurations.
	 */
	@BeforeClass
	public static void setupTestEnvironment() {
		DefaultEditorDialog.setDefault("*.yaml", GTE);

		ImportIntoWorkspace.importFolder(PROJECT_FOLDER_PATH);

		RunConfigurationDialog.createMaven(MVN_CONF, "${workspace_loc:/main-yaml}");
		DebugConfigurationDialog.createCTD(CTD_CONF);
	}
	
	/**
	 * Remove breakpoints, run/debug configurations and clean workspace.
	 */
	@AfterClass
	 ...
	 */
	@Test
	public void YAMLDSLTest() {
		
		// Open file with camel route.
		ProjectExplorer explorer = new ProjectExplorer();
		explorer.open();
		ProjectItem routeFile = explorer.getProject("main-yaml").getProjectItem("src", "main", "resources", "routes",
				"my-route.yaml");
		routeFile.open();
